...
반복되는 테스트 구문을 줄이기 위해 `before`및 `after` 훅을 사용할 수 있습니다. 테스트 파일 내에서 `before` 훅을 추가하면, 각 테스트 전에 공통 작업을 실행할 수 있습니다. 이러게 하면 각 테스트가 다른 테스트에 의존하지 않고 독립적으로 동작할 수 있습니다.
...
CI에서만큼은 필요한 브라우저만 설치하세요. 만약 Chronium만 테스트한다면, Chronium만 설치하세요.
